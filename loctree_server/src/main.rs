...
impl LoctreeServer {
    ...
    async fn slice(&self, Parameters(params): Parameters<SliceParams>) -> String {
        ...
        let Some(target) = snapshot
            .files
            .iter()
            .find(|f| f.path.ends_with(&params.file))
        else {
            return format!("File not found: {}", params.file);
        };
    ...
    #[tool(
        name = "find",
        description = "Find where a function/class/type is defined. USE THIS BEFORE creating anything new - avoid duplicates. Shows all matches with file and line number."
    )]
    async fn find(&self, Parameters(params): Parameters<FindParams>) -> String {
        ...
        let regex = match regex::Regex::new(&params.name) {
            Ok(r) => r,
    ...
    async fn impact(&self, Parameters(params): Parameters<ImpactParams>) -> String {
        ...
        let direct: Vec<_> = snapshot
            .edges
            .iter()
            .filter(|e| e.to.ends_with(&params.file))
            .map(|e| e.from.clone())
            .collect();
    ...
    async fn trace(&self, Parameters(params): Parameters<TraceParams>) -> String {
        ...
        let matching_bridges: Vec<_> = snapshot.command_bridges.iter()
            .filter(|b| {
    ...
    async fn tree(&self, Parameters(params): Parameters<TreeParams>) -> String {
        ...
        large_files.sort_by(|a, b| {
            b.get("loc").and_then(|v| v.as_u64()).unwrap_or(0)
